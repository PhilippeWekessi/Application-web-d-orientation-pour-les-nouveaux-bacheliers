...
<head>
  <meta charset="UTF-8"/>
  <title>Modifier Filière — Admin OrientaBac</title>
  ...
  <div class="main">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <p>Modifier les informations de la filière</p>
      </div>
    ...
    <div class="content">
      <div class="panel">
        <div class="panel-header">
          <h3>Informations de la filière</h3>
        </div>
        <form method="POST" action="{{ route('admin.filieres.update', $filiere->id_filiere) }}">
          @csrf @method('PUT')
          <div style="display: grid; grid-template-columns: 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
              <label style="display:block; margin-bottom:5px; font-weight:bold;">Nom</label>
              <input type="text" name="nom" value="{{ $filiere->nom }}" required style="width:100%; padding:10px; border:1px solid #ddd; border-radius:6px;">
            </div>
            <div>
              <label style="display:block; margin-bottom:5px; font-weight:bold;">Description</label>
              <textarea name="description" required style="width:100%; padding:10px; border:1px solid #ddd; border-radius:6px; min-height:100px;">{{ $filiere->description }}</textarea>
            </div>
            <div>
              ...
              <select name="statut" required style="width:100%; padding:10px; border:1px solid #ddd; border-radius:6px;">
                <option value="en_attente" {{ $filiere->statut == 'en_attente' ? 'selected' : '' }}>En attente</option>
                <option value="validee" {{ $filiere->statut == 'validee' ? 'selected' : '' }}>Validée</option>
                <option value="rejetee" {{ $filiere->statut == 'rejetee' ? 'selected' : '' }}>Rejetée</option>
              </select>
            </div>
          </div>
          <button type="submit" class="btn-panel">Mettre à jour</button>
          <a href="{{ route('admin.filieres') }}" class="btn-panel-outline">Annuler</a>
        </form>
      </div>
    </div>
  </div>
